...
export function calculateMaintenanceScore(metrics: RepositoryMetrics, isArchived: boolean): number {
  ...
  if (metrics.openIssuesCount !== null) {
...
export function calculateCommunityScore(metrics: RepositoryMetrics): number {
  ...
  if (metrics.hasContributing === true) score += 25;
  if (metrics.hasCodeOfConduct === true) score += 20;

  if (metrics.subscribersCount !== null) {
    if (metrics.subscribersCount >= 100) score += 25;
    else if (metrics.subscribersCount >= 10) score += 15;
  }

  if (metrics.forksCount !== null) {
...
export function calculateCodebaseScore(metrics: RepositoryMetrics): number {
  ...
  const topicsCount = metrics.topics?.length ?? 0;
  if (topicsCount >= 5) score += 25;
  else if (topicsCount >= 1) score += 15;

  if (metrics.sizeInKb !== null && metrics.sizeInKb > 50 && metrics.sizeInKb < 1000000) score += 20;
...
export function calculateDocumentationScore(metrics: RepositoryMetrics, summary: RepositorySummary): number {
  ...
  if (metrics.hasReadme === true) score += 50;
...
export function calculatePopularityScore(metrics: RepositoryMetrics): number {
  ...
  const forkBonus = metrics.forksCount !== null && metrics.forksCount > 100
    ? 10
    : metrics.forksCount !== null && metrics.forksCount > 10
      ? 5
      : 0;
...
export function calculateHealthScore(metrics: RepositoryMetrics, isArchived: boolean): number {
  ...
  if (metrics.hasReadme === true) score += 25;
  if (metrics.hasLicense === true) score += 25;
  if (metrics.hasContributing === true) score += 25;
  if (metrics.hasCodeOfConduct === true) score += 25;
...
export function calculateOverallScore(categoryScores: Omit<RepositoryScores, 'overall'>): number {
  const weightedSum =
    categoryScores.activity * SCORE_WEIGHTS.activity +
    categoryScores.maintenance * SCORE_WEIGHTS.maintenance +
    categoryScores.documentation * SCORE_WEIGHTS.documentation +
    categoryScores.community * SCORE_WEIGHTS.community +
    categoryScores.popularity * SCORE_WEIGHTS.popularity +
    categoryScores.codebase * SCORE_WEIGHTS.codebase +
    categoryScores.health * SCORE_WEIGHTS.health;
...
export function computeRepositoryScores(metrics: RepositoryMetrics, summary: RepositorySummary): RepositoryScores {
  const isArchived = summary.isArchived === true;
